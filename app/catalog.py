...
def build_catalog_block(
    ...
    *,
    ...
    Sections appear only when populated: Services, Products, Opening hours, then Key
    facts from the 'content' snippets. Callers pass items in display order (kind,
    category, name) — this function only formats. `hours_text` comes from
    format_hours_text, so "when are you open?" is answered from curated data instead of
    whichever crawled page happened to mention a time.
    """
...
async def load_business_context(tenant_id: str) -> dict[str, Any]:
    ...
    return {
        "catalog_block": build_catalog_block(
            items, content, hours_text=format_hours_text(hours_from_profile(profile))
        ),
